...
def getArticles():
    ...
    fromDate = datetime.datetime.combine(datetime.datetime.now() + datetime.timedelta(days=-1), datetime.time.min)
...
def getArticlesForBot():
    ...
    cur.execute('''
    select id, feed_name, feed_url, title, url from t_article where status = 0 order by updated_at desc limit 100
    ''')
